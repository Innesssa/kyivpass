...
class InfoTag {	
    ...
    function admTag() {
        ...
        if($com=="list"){
            ...
            if($event!=""){
                $tag_data=InfoTag::tagData($id);

                if($tag_data["level"]==1){
                    InfoTag::Event1($event,MOD_TAG_TABLE,$id);
                }
                InfoTag::Event2($event,MOD_TAG_TABLE,$id);
            }
	
            $tag_list1=InfoTag::tagListIdData($level=1);
            $tag_list2=InfoTag::tagListIdData($level=2);
...
//SYS::varDump($level.__FILE__,__LINE__,"level");	
            ...
            if($id!=""&&$id!="new"){
                ...
                if($arr_value['level']==2){
	                $tag_connect_list=InfoTag::tagListConnectData($id);
                    ...
                    if($pc!=""){
                        foreach($pc as $key=>$val){
                            $c_tag_data=InfoTag::tagData($val);
