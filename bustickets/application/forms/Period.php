...
class Application_Form_Period extends Zend_Form
{
    public function init()
    {
        ...
        $elm->addValidator('NotEmpty', true, array('messages' => array('isEmpty' =>  'Введіть початок дії')))
        ...
        $elm->setRequired(true);
        $elm->setLabel('Дата відправлення: ');
        $elm->addValidator('NotEmpty', true, array('messages' => array('isEmpty' =>  'Введіть завершення дії')))
            ->addValidator('Regex',true, array('/^\d{2}.\d{2}\.\d{4}/'))
        ;
        $elm->setAttrib("class", "input-middle datepicker");
        $elm->setAttrib("maxlength", "10");
        ...
        $elm = new Zend_Form_Element_Button('getreport');
        $elm->setLabel('Звіт');
