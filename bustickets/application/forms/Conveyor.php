...
class Application_Form_Conveyor extends Zend_Form
{
    ...
    public function init()
    {
        ...
        $elm->setRequired(false)
            ...
            ->addValidator('StringLength',true,array('min'=>0,'max'=>20,
                ...
                'messages' => array(
                    Zend_Validate_Db_NoRecordExists::ERROR_RECORD_FOUND => 'Такий код  вже присутній')
            )) ;
        ...
        $elm = new Zend_Form_Element_Select('insurerid');
        $elm->setLabel("Страхова компанія: ");
        $elm->setRequired(true)
            ->addValidator('Digits', true,array(
                'messages' => array(
                    Zend_Validate_Digits::NOT_DIGITS         =>  'Оберіть Страхову компанію',
                    Zend_Validate_Digits::STRING_EMPTY       =>  'Оберіть Страхову компанію'),
                Zend_Validate_Digits::INVALID            =>  'Оберіть Страхову компанію'))
        ...
        $elm = new Zend_Form_Element_Text('insurerrate');
        $elm->setRequired(false)
            ->setLabel('Страхова ставка % : ')
            ->addValidator('StringLength',true,array('min'=>0,'max'=>4,
                'messages'=>array(
        ...
        $elm = new Zend_Form_Element_MultiCheckbox('benefits',array(
            'separator'       => '',           // to keep everything inline
            'label_placement' => 'prepend',
            'label_class'     => 'box-inline'));
        $elm->setLabel("Пільги: ");
        $dbB = new Application_Model_DbTable_Benefitslist();
        $rs = $dbB->fetchAll(null,'title_short ASC');
        if($rs) foreach($rs as $r){
            $elm->addMultiOption($r->id,$r->title_short."(".$r->benefit_perc.")");
        }
        /*
        $option = array(
        ...
        $elm->addMultiOptions($option);
        */
        $elm->setDecorators(array(
            'ViewHelper',
            'Errors',
            array(array('data' => 'HtmlTag'), array('tag' => 'div', 'class'  => 'controls-full')),
            array('Label',array('class'=>'control-label')),
            array(array('row' => 'HtmlTag'), array('tag' => 'div', 'class' => 'control-group')),
        ));
        $this->addElement($elm);
